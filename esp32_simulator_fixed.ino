...
#define BACKEND_HOST     "https://esp32simulator-production.up.railway.app"
...
void setup() {
  ...
  // Test DNS resolution
  IPAddress ip;
  if (WiFi.hostByName("esp32simulator-production.up.railway.app", ip)) {
    Serial.print("[DNS] Resolved to: ");
    Serial.println(ip);
  } else {
    Serial.println("[DNS] Failed to resolve domain");
  }
